...
import { ApprovalStatus, ReimburseResponse } from "@/app/lib/types/reimburse/reimburseTypes";

const ReimburseData = ({ selectedBulan }: { selectedBulan: string }) => {
    ...
    const { data } = useReimburse().fetchAllReimburse({
        page: 1,
        limit: 1000,
        minSubmittedDate: monthMin,
        maxSubmittedDate: monthMax,
    });

    const stats = useMemo(() => {
        const reimburses: ReimburseResponse[] = data?.data || [];
        const today = new Date();

        const calculateAmount = (list: ReimburseResponse[]) => 
            list.reduce((acc, curr) => acc + (curr.totalExpenses || 0), 0);

        const paidSalaries = reimburses.filter(s => s.approvalStatus === ApprovalStatus.APPROVED);
        const pendingSalaries = reimburses.filter(s => s.approvalStatus === ApprovalStatus.PENDING);
        const overdueSalaries = reimburses.filter(s => s.approvalStatus === ApprovalStatus.REJECTED);

        return [
            { 
                id: 1, 
                label: "Total Data Reimburse", 
                count: reimburses.length, 
                amount: calculateAmount(reimburses),
                color: "bg-blue-100 text-blue-700" 
            },
            { 
                id: 2, 
                label: "Reimburse Sudah Dibayar", 
