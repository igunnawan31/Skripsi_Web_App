    ...
    const fetchAllKontrak = (filters?: {
        ...
        return useQuery({
            ...
            queryFn: async () => {
                ...
                if (filters?.metodePembayaran) queryParams.append("status", filters.metodePembayaran);
                ...
                const response = await fetch(`${API}/kontrak?${queryParams.toString()}`, {
                    method: "GET",
                    headers: {
                ...
                if (!response.ok) {
                    let errorMessage = "Failed to fetch kontrak"
                    try {
