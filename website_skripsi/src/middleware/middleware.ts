...
export function middleware(req: NextRequest) {
    ...
    if (token && isProtected) {
        if (majorRole === MajorRole.OWNER && !pathname.includes("(owner)")) {
            return NextResponse.redirect(new URL("/dashboard/(owner)", req.url));
        }
        if (majorRole === MajorRole.KARYAWAN && minorRole === MinorRole.HR && !pathname.includes("(hr)")) {
...
export const config = {
  matcher: ["/dashboard/:path*"], // only guard dashboard area
};
