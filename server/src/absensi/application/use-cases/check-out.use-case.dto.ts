...
import { CheckOutDTO } from '../dtos/request/check-out.dto';
...
export class CheckOutUseCase {
  ...
  async execute(
    ...
    const now = new Date();
    const updatedAbsensi = await this.absensiRepo.checkOut(userId, today);
    ...
    this.eventEmitter.emit(
      'absensi.checkedOut',
      new AbsensiCheckedOutEvent(userId, today, new Date(absensi.checkIn), now, duration),
    );
